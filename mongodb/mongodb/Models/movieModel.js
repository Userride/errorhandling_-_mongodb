...
const movieSchema = new mongoose.Schema({
    name: {
        ...
        trim: true,//no whitespace b/w movies
    ...
    ratings: {
        ...
        validate: {
            validator: function(value){
                return value >= 1 && value <= 10;  //minmum rating 1
            },
            message: "Ratings ({VALUE}) should be above 1 and below 10"
        }
    },
    totalRating: { //how many customber rated movies
        type: Number
    },
    releaseYear: {
        type: Number,
    ...
    toJSON: {virtuals: true}, //we want virtual properties as output ..,
    toObject: {virtuals: true}
});

//virtual properties to change timr min to hours
movieSchema.virtual('durationInHours').get(function(){
    return this.duration / 60;
})

//EXECUTED BEFORE THE DOCUMENT IS SAVED IN DB
//.save() or .create() ->ye dono rahe ga tb he save method kaam kare ga..
//inserMany, findByIdAndUpdate will not work
// document middle ware
movieSchema.pre('save', function(next) {
    this.createdBy = 'Prince singh';
    next();
})

//document is already save in database
...
//query midddleware-->find method query all movies
movieSchema.pre(/^find/, function(next){
    this.find({releaseDate: {$lte: Date.now()}});//this keyword call the find method to get all movies
